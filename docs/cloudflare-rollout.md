...
- Backup hostname: `oldblog.hichee.com` resolves to Cloudways server `149.248.0.88`; post-cutover checks on 2026-06-02 HST saw ports 80/443 time out, so re-verify Cloudways reachability before relying on it as a live rollback endpoint.
...
- Post-cutover live media audit: all 20,969 generated `/wp-content/*` URLs returned partial-content success from both `https://blog.hichee.com` and `https://newblog.hichee.com`.
- Production canonical/share metadata now uses `https://blog.hichee.com`; `newblog.hichee.com` remains an alias/staging comparison domain.
...
8. Verify the backup hostname serves `/wp-content/*` media directly. Completed during setup on 2026-06-02; after cutover, direct checks to `oldblog.hichee.com` timed out, so fix/re-verify Cloudways reachability before depending on this hostname for rollback.
...
- If critical issue appears, first verify the Cloudways origin responds, then switch DNS/custom domain back to WordPress origin.
